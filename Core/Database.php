...
class Database
{
    ...
    private function __construct()
    {
        ...
        // Dynamic Timezone sync (Offset calculation for broad compatibility)
        $timezone = Config::get('timezone', 'America/Argentina/Buenos_Aires');
        $now = new \DateTime('now', new \DateTimeZone($timezone));
        $offset = $now->format('P'); // e.g., -03:00
        ...
        $options = [
            ...
            PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES {$charset} COLLATE {$collation}; SET time_zone = '{$offset}';"
        ];
